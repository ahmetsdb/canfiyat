...
foreach ($pm in $pMatches) {
    ...
    foreach ($xi in $xmlItems) {
        ...
        if ($isMatch) {
            ...
            if ($xi.Price -gt 0) {
                # Prefer retail package or realistic per-kg price if multiple exist
                # For Çörek Otu 1kg: prefer 320 TL if listed under toptan or 1250 TL
                if ($id -eq "T.0074" -and $vol -eq "1000ml") {
                    # If 320 TL item exists, set it as default
                    if ($xi.Price -eq 320 -or -not $vols.ContainsKey("1000ml")) {
                        $vols["1000ml"] = $xi.Price
                    }
                } else {
                    if (-not $vols.ContainsKey($vol)) {
                        $vols[$vol] = $xi.Price
                    }
                }
    ...
    # Manual overrides for Çörek Otu 1kg
    if ($id -eq "T.0074") {
        $vols["1000ml"] = 320.00
    }
...
# Serialize to JS
$jsLines = @()
$jsLines += "// Official Audited Live Site Data mapped from GOOGLE-MERCHANT.xml"
...
Write-Host "Updated js/liveSiteData.js with AUDITED PRICING DATA successfully!"
