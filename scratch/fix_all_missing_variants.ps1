...
# Accurate Multi-Stem Mappings for all 65 products
$stemMap = @{
    "U.0271" = @("yasemin")
    "U.0326" = @("bergamot")
    "U.0235" = @("biberiye")
    ...
    "T.0407" = @("defne yaprag", "defne-yaprag")
    "U.0332" = @("greyfurt")
    "U.0106" = @("karanfil")
    "U.0105" = @("karanfil")
    "U.0199" = @("nane")
    ...
    "U.0176" = @("vanilya")
    "T.0243" = @("argan")
    "T.0097" = @("at kestanes", "at-kestanes")
    "T.0245" = @("avokado")
    "T.0148" = @("aynisefa", "kalendula")
    "T.0078" = @("badem", "tatli-badem", "tatli badem")
    "T.0254" = @("bamya")
    "T.0013" = @("bugday ruseym", "bugday-ruseym", "bugday oz")
    "T.0147" = @("chia")
    "T.0074" = @("corek otu", "corek-otu", "corek")
    "T.0363" = @("cuha")
    "T.0353" = @("defne tohum", "defne-tohum")
    "T.0323" = @("deve dikeni", "deve-dikeni")
    "T.0213" = @("hashas")
    "T.0077" = @("hindistan ceviz", "hindistan-ceviz")
    "T.0155" = @("hint yag", "hint-yag")
    "T.0364" = @("hodan")
    "T.0366" = @("isirgan")
    "T.0362" = @("incir cekirdeg", "incir-cekirdeg")
    "T.0110" = @("jojoba")
    "T.0080" = @("kabak cekirdeg", "kabak-cekirdeg")
    "T.0224" = @("kakao")
    "T.0082" = @("kayisi cekirdeg", "kayisi-cekirdeg", "kayisi")
    "T.0209" = @("kenevir", "kendir")
    "T.0083" = @("keten tohum", "keten-tohum", "keten")
    "T.0104" = @("kusburnu")
    "T.0270" = @("makademya")
    "T.0210" = @("menengic", "bittim")
    "T.0084" = @("nar cekirdeg", "nar-cekirdeg", "nar")
    "T.0340" = @("pirinc kepeg", "pirinc-kepeg")
    "T.0081" = @("sari kantaron", "sari-kantaron", "kantaron")
    "T.0246" = @("sarimsak")
    "T.0355" = @("shea")
    "T.0085" = @("susam")
    "T.0365" = @("tamanu")
    ...
    "T.0086" = @("uzum cekirdeg", "uzum-cekirdeg", "uzum")
    "T.0321" = @("visne")
    "T.0125" = @("kudret nar", "kudret-nar")
    "T.0221" = @("kudret nar", "kudret-nar")
    "T.0389" = @("zeytinyag")
    "A.0200" = @("bugday")
}
...
foreach ($pm in $pMatches) {
    ...
    $stems = $stemMap[$id]
    if (-not $stems) {
        $stems = @((Normalize-Tr ($name.Split(' ')[0])))
    }
    ...
    foreach ($xi in $xmlItems) {
        ...
        $cleanLink  = Normalize-Tr $xi.Link
        
        $isMatch = $false
        foreach ($st in $stems) {
            $normSt = Normalize-Tr $st
            if ($cleanTitle.Contains($normSt) -or $cleanLink.Contains($normSt)) {
                $isMatch = $true
                break
            }
        }
        ...
        if ($isMatch) {
            # Skip multi-pack combos like 2li Paket
            if ($xi.Title -match '2li Paket|2 adet|5 kg|10 kg|toptan') {
                # skip bulk combo unless no retail volume
            }
            ...
            elseif ($xi.Title -match '(?i)20\s*ml') { $vol = "20ml" }
                ...
                # If no volume mentioned in title, check item base volume
                if ($xi.Title -notmatch 'ml|gr|kg') {
                    $vol = "250ml"
                }
            }

            if ($xi.Price -gt 0 -and $xi.Price -lt 15000) {
                if (-not $vols.ContainsKey($vol) -or ($vols[$vol] -gt 1500 -and $xi.Price -lt 1500)) {
                    $vols[$vol] = $xi.Price
                }
...
Write-Host "`n=== COMPREHENSIVE MATCH SUMMARY ==="
Write-Host "Cansızzade Sitede Bulunan Ürün Sayısı:" $foundCount
Write-Host "Cansızzade Sitede Olmayan (N/A):" $notFoundCount

# Serialize to JS
$jsLines = @()
$jsLines += "// Official Comprehensive Live Site Data mapped from GOOGLE-MERCHANT.xml"
...
Write-Host "Updated js/liveSiteData.js with COMPREHENSIVE MATCH DATA successfully!"
