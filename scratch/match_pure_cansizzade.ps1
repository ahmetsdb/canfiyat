...
# Parse XML items, strictly filtering OUT Endora
...
foreach ($im in $itemMatches) {
    ...
    # Check if Endora
    $isEndora = ($title -like "*Endora*" -or $title -like "*endora*" -or $link -like "*endora*" -or $brand -like "*Endora*")
...
# Key stem map for all 65 products
$stemMap = @{
    "U.0271" = "yasemin"
    "U.0326" = "bergamot"
    "U.0235" = "biberiye"
...
foreach ($pm in $pMatches) {
    ...
    if (-not $stemPattern) {
        $stemPattern = Clean-Str ($name.Split(' ')[0])
    }
    ...
    foreach ($xi in $xmlItems) {
        $cleanTitle = Clean-Str $xi.Title
        
        $isMatch = $false
        $patterns = $stemPattern.Split('|')
        foreach ($pat in $patterns) {
            if ($cleanTitle.Contains($pat)) {
                $isMatch = $true
                break
            }
        }
    ...
    if ($hasPrice) {
        ...
        Write-Host "✅ [PURE CANSIZZADE] $sku - $name -> Sitedeki Ambalajlar: $vStr"
    } else {
        $notFoundCount++
...
Write-Host "`n=== PURE CANSIZZADE MATCH SUMMARY ==="
Write-Host "Pure Cansızzade Sitede Bulunan Ürün Sayısı:" $foundCount
Write-Host "Cansızzade Fiyatı Olmayan (N/A Gösterilecek):" $notFoundCount

# Serialize to JS
$jsLines = @()
$jsLines += "// Official Pure Cansızzade Data mapped from GOOGLE-MERCHANT.xml (ENDORA EXCLUDED)"
...
Write-Host "Updated js/liveSiteData.js with PURE CANSIZZADE DATA successfully!"
