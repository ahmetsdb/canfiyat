...
# Accurate Product Stem Mappings
$stemMap = @{
    "U.0271" = @("yasemin")
    "U.0326" = @("bergamot")
    "U.0235" = @("biberiye")
    ...
    "T.0078" = @("badem", "tatli badem")
    "T.0254" = @("bamya")
    "T.0013" = @("bugday ruseym", "bugday oz")
    "T.0147" = @("chia")
    ...
    "T.0084" = @("nar cekirdeg", "nar")
    "T.0340" = @("pirinc kepeg")
    "T.0081" = "sari kantaron"
    "T.0246" = @("sarimsak")
    "T.0355" = @("shea")
    "T.0085" = @("susam")
    "T.0365" = @("tamanu")
...
foreach ($pm in $pMatches) {
    ...
    foreach ($xi in $xmlItems) {
        ...
        if ($isMatch) {
            # Skip multi-pack combo packages like 2li Paket
            ...
            elseif ($xi.Title -match '(?i)20\s*ml') { $vol = "20ml" }
                ...
                # If no volume specified in title, check default volume or price range
                if ($xi.Title -notmatch 'ml|gr|kg') {
                    $vol = "250ml"
                }
            }

            if ($xi.Price -gt 0 -and $xi.Price -lt 10000) {
                # Store retail price for volume
...
Write-Host "Updated js/liveSiteData.js with PERFECT MATCH DATA successfully!"
