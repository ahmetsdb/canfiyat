...
foreach ($pm in $pMatches) {
    ...
    foreach ($xi in $xmlItems) {
        ...
        if ($isMatch) {
            ...
            if ($xi.Title -match '2li Paket|2 adet') { continue }
            ...
            if ($xi.Price -gt 0) {
                # Strict Retail Price selection from XML:
                # For 1kg (1000ml), pick retail title price (e.g. 1250 TL for Çörek Otu)
                if (-not $vols.ContainsKey($vol) -or $xi.Price -gt $vols[$vol]) {
                    # If retail bottle entry exists (higher/standard retail price), use it
                    $vols[$vol] = $xi.Price
                }
                if (-not $matchLink -or $xi.Title -match '(?i)1\s*kg|250\s*ml') { $matchLink = $xi.Link }
            }
        }
    }
...
# Serialize to JS
$jsLines = @()
$jsLines += "// Official Pure XML Retail Data mapped from GOOGLE-MERCHANT.xml"
...
Write-Host "Updated js/liveSiteData.js with PURE XML RETAIL DATA successfully!"
