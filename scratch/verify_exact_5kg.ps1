...
foreach ($im in $itemMatches) {
    ...
    # Match ONLY title explicitly saying "5 kg" or "5kg"
    if ($title -match '\b5\s*kg\b') {
        $priceVal = 0.0
        [double]::TryParse($price, [ref]$priceVal) | Out-Null
        $exact5kg += [PSCustomObject]@{
            Title = $title.Trim()
            Price = $priceVal
            Link = $link.Trim()
        }
...
Write-Host "=== EXACT 5 KG ITEMS IN GOOGLE-MERCHANT.XML ==="
Write-Host "Total Explicit 5 KG Items:" $exact5kg.Count

$exact5kg | Sort-Object Title | ForEach-Object {
